    ...
    postProcess(context) {
        Object.keys(this.dictionary).forEach(key => {
            context.text = context.text.replace(new RegExp("( " + key + " )|(" + key + " )|( " + key + ",)|( " + key + "\\.)|( " + key + "\\!)|( " + key + "\\?)", 'gi'), (match) => {
                if (match[0] == ' ') {
                    return " " + Helpers.matchCasing(this.dictionary[key], match.substring(1)) + " ";
                } 
                //The start of the text.
                else if (Helpers.startIsUpper(match))
                    return Helpers.matchCasing(this.dictionary[key], match) + " ";
                else return match
            })
        })
