...
export class SmartDictionaryGenderMiddleware {
  ...
  /**
    ...
    */
  postProcess (context) {
    if (!('words' in context.data)) { throw new Error('Missing context.data.words!') }
    ...
    for (let w = 0; w < context.data.words.length; w++) {
      ...
      if ((Helpers.firstLetterUpper(word) + ';False' in dicNouns.wordlist) ||
        ...
        const maleArticles = this.getMaleArticlesForNoun(context, w)
        if (Helpers.firstLetterUpper(word) + ';False' in dicNouns.wordlist) {
          ...
          if (Helpers.firstLetterUpper(word) + ';True' in dicNouns.wordlist) {
            const plural = maleArticles.length === 0
        ...
        } else { context.data.words[w] = Helpers.matchCasing(dicNouns.wordlist[Helpers.firstLetterUpper(word) + ';True'], context.data.words[w]) }
  ...
  getMaleArticlesForNoun (context, nounIndex) {
    ...
    for (let o = -1; o < 2; o += 2) {
      ...
      for (let wb = nounIndex + o;
        ...
        if (context.data.words[wb].toLowerCase() in this.articles) { liArticleIndexes.push(wb) }
